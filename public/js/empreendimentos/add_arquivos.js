...
const uppy = Uppy.Core({
    autoProceed: false,
    restrictions: {
        allowedFileTypes: ['.txt', '.doc', '.avi','.bmp','.csv','.docx','.epub','.mobi', '.mov', 
        '.mp3','.mp4','.pdf','.ppt','.pptx','.rtf','.wav','.wmv',
        '.xls','.xlsx','image/*']
      },
})
uppy.use(Uppy.Dashboard, {
    target: '#upload_emp_arquivos',
    inline: true,
...
uppy.use(Uppy.XHRUpload, {
    endpoint: baseURL + "/admin/empreendimentos/upload_arquivos/" + $("#id_pasta").data("target")+'/' + $("#id_empreendimento").data("target"),
    headers: {
...
uppy_img_principal.use(Uppy.XHRUpload, {
    endpoint: baseURL + "/admin/empreendimentos/upload_img_principal/" + $("#id_empreendimento").data("target"),
    headers: {
        "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content")
    },
    formData: true,
